...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  for(;featuresarrayisready==false;)//arrayin tamamını doldurup doldurmadığını check ediyo
		{
		  	if(drdyFlag==1){ //data geldiyse 1 oluyor
		  		drdyFlag=0; // data işleniyorsa 0 oluyor
		  		myData = LIS3DSH_GetDataScaled(); // x-y-z datalarını alıyorum
		  		  	features[3*countt]=myData.x;// x değerini array e atıyorum
		  		  	features[3*countt+1]=myData.y; // y değerini array e atıyorum
		  		  	features[3*countt+2]=myData.z;// z değerini array e atıyorum
		  		  	countt++;
			  if(countt==EI_CLASSIFIER_RAW_SAMPLE_COUNT){ // array dolduysa başa gel baştan başla
						countt=0; // erişmek istediğim datanın değerini veriyorum
